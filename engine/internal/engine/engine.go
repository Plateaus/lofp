...
func (e *GameEngine) ProcessCommand(ctx context.Context, player *Player, input string) *CommandResult {
	...
	case "LEARN":
		return e.doLearnScroll(ctx, player, args)
	case "COMMAND":
...
func (e *GameEngine) doChant(ctx context.Context, player *Player, args []string) *CommandResult {
	...
	for i, ii := range player.Inventory {
		...
		if matchesTarget(name, target, e.getAdjName(ii.Adj1)) {
			...
			if spellID <= 0 {
				return &CommandResult{
					Messages: []string{"The scroll appears to be blank."},
				}
			}

			fullName := e.formatItemName(itemDef, ii.Adj1, ii.Adj2, ii.Adj3)

			// Prepare the spell stored in VAL3.
			player.PreparedSpell = spellID
			...
			return &CommandResult{
				Messages: []string{
					"As you chant the scroll, it crumbles into dust...",
					"You feel the power of the scroll flow into you.",
					fmt.Sprintf("You prepare the spell '%s'.", spell.Name),
					"[Round: 3 sec]",
				},
				RoomBroadcast: []string{
					fmt.Sprintf(
						"%s chants from %s, which crumbles into dust.",
						player.FirstName,
						fullName,
					),
				},
...
func (e *GameEngine) doLearnScroll(ctx context.Context, player *Player, args []string) *CommandResult {
	if len(args) == 0 {
	...
	for i, ii := range player.Inventory {
		...
		if !strings.Contains(strings.ToUpper(def.Type), "SCROLL") {
			continue
		}
		...
		if spell == nil {
			return &CommandResult{
				Messages: []string{"The scroll contains no magic you can understand."},
			}
		}
		...
		if player.KnownSpells[spell.ID] {
			return &CommandResult{
				Messages: []string{fmt.Sprintf("You already know '%s'.", spell.Name)},
			}
		}
		...
		return &CommandResult{
			Messages: []string{
				"You study the scroll carefully.",
				fmt.Sprintf("You learn the spell '%s'.", spell.Name),
	...
	return &CommandResult{
		Messages: []string{"You don't have a matching scroll."},
	}
}
